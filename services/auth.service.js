...
class AuthService {
  async signup(body) {
    ...
    if (existedUser) throw new Error('User has already signed up')
  ...
  async signin(body) {
    const existedUser = await userModel.findOne({ email: body.email })
    if (!existedUser) throw new Error('User has not signed up yet')
  ...
  async refresh(refreshToken) {
    ...
    if (!userPayload || !foundedRefreshToken) throw new Error('Unauthorized error')
  ...
  async forgotPass(email) {
    ...
    if (!existedUser) throw new Error('User has not signed up with this email')
  ...
  async setNewPass(accessToken, pass) {
    ...
    if (!userPayload) throw new Error('Unauthorized error')
